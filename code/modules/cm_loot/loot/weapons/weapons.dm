...
/datum/loot_entry/weapon
	name = "weapon item"
	var/item_to_spawn
	var/datum/loot_table/possible_variants

/datum/loot_entry/weapon/New()
	. = ..()
	possible_variants = GLOB.loot_variants

/datum/loot_entry/weapon/spawn_item(var/atom/spawn_location)
	if(!item_to_spawn)
		return

	var/obj/item/weapon/gun/G = new item_to_spawn(spawn_location)

	if(!istype(G))
		return

	if(G.flags_gun_features & GUN_UNUSUAL_DESIGN)
		return

	var/datum/loot_entry/variant/V = SSloot.generate_loot(possible_variants)
	if(!V)
		return
